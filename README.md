...
- 独立是非判断模式：一次 XZKB 混合应用调用后驱动舵机或网页动画，不生成 TTS
- 网页判断会话与物理设备、其他网页用户的状态和动作输出完全隔离
...
是非判断不进入普通讲解上下文，也不命中讲解高频缓存。应用配置、固定 JSON 协议和上线样例见 [XZKB 混合是非判断应用](docs/verdict-application.md)。
...
当前代码已完成网页文字问答、WAV 上传、树莓派本地麦克风语音闭环、单按钮三模式状态机和舵机动作适配。没有实体舵机时可在设备测试页使用隔离的网页动画验证判断流程。
...
在 `/device-test` 填写设备密钥后，通过同一个主按钮控制现场交互。物理按钮长按 1.5 秒按“正常对话 → 是非判断 → 知识补充 → 正常对话”循环切换，短按开始或停止当前模式录音。等待知识确认时，短按重新录入，长按保存并返回正常对话。

网页测试区提供同级的“对话 / 是非判断 / 知识补充”切换。网页是非判断使用当前浏览器麦克风或 WAV 文件，并把 ASR 文本发送给当前网页会话的判断工作流；页面只播放模拟动画，绝不驱动实体 GPIO。实体按钮的判断模式则使用树莓派麦克风和实体舵机。两者共享判断规则，但不共享状态、轮次或动作输出。
...
是非判断只展示机械结果：思考时缓慢左右摇摆，明确结果先向反方向蓄势再快速到“是”或“否”并保持；无效问题、依据不足、高风险或服务失败回到中立。明确判断不会合成或播放普通语音回答。
