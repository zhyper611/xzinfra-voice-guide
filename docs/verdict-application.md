...
7. 对于“某人说的对吗/对不对”这类省略具体言论的问题，必须先检索知识库。如果知识库中存在同一主体“说的对”或“说的不对”的明确陈述，应将该陈述视为充分依据并据此返回 yes 或 no，不得仅因问题未复述具体言论而判为 invalid。
8. casual 和 invalid 的 evidence 必须为空字符串。neutral 不得伪造知识依据。
9. reason 只说明本次判断原因，不向用户提问，不输出建议清单，不生成口播回答。
10. 输出必须能被标准 JSON 解析器直接解析，所有五个字段都必须存在且为字符串。
```
